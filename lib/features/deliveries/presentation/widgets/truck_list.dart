...
class TruckList extends ConsumerStatefulWidget {
  const TruckList({
    super.key,
  });

  @override
  ConsumerState<TruckList> createState() => _TruckListState();
}
...
class _TruckListState extends ConsumerState<TruckList> {
  ...
  void _handleConfirmDelivery(BuildContext context) {
    if (_isNavigating || !mounted) {
      debugPrint(
          'Truck List - Navigation already in progress or widget not mounted');
      return;
    }
    ...
    if (state != null && state.truck.products.isNotEmpty) {
      ...
      Navigator.push(
        ...
        // Reset navigation state when returning from confirm delivery
        if (mounted) {
          setState(() {
            _isNavigating = false;
          });
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    if (!mounted || _isNavigating) {
      debugPrint(
          'Truck List - Key event ignored: mounted=$mounted, navigating=$_isNavigating');
    ...
    try {
      ...
      if (event is KeyDownEvent &&
        ...
        debugPrint('Truck List - Key pressed: ${event.logicalKey}');
        // Check if truck is not empty before proceeding
        if (deliveryState.valueOrNull?.truck.products.isNotEmpty == true) {
          debugPrint(
              'Truck List - Enter pressed, proceeding to confirm delivery');
          _handleConfirmDelivery(context);
          return KeyEventResult.handled;
        } else {
          debugPrint('Truck List - Enter pressed but truck is empty');
        }
      }
    } catch (e) {
