...
class ExpensesSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ExpensesSheet> createState() => _ExpensesSheetState();
}

class _ExpensesSheetState extends ConsumerState<ExpensesSheet> {
  ...
  Future<void> _applyPendingChanges() async {
    ...
    try {
      ...
      if (cellsToUpdate.isNotEmpty) {
        ...
        await ref.read(expensesProvider.notifier).updateCells(cellsToUpdate);
      }

      // Process creates in bulk if any
      if (cellsToCreate.isNotEmpty) {
        print("Creating ${cellsToCreate.length} cells");
        await ref.read(expensesProvider.notifier).createCells(cellsToCreate);
      }
  ...
  @override
  void didUpdateWidget(ExpensesSheet oldWidget) {
  ...
  Future<void> _deleteRow(String rowId) async {
    try {
      await ref.read(expensesProvider.notifier).deleteRow(rowId);
    } catch (e) {
            ...
            child: Column(
              ...
              children: [
                ...
                _buildFormulaOption(
                  ...
                  // Icon should be a multiply sign
                  Icons.clear,
                  () {
                    if (columnIndex >= 2) {
                      String formula =
